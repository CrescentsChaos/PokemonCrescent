...
async def spawn(ctx:discord.Interaction):
    ...
    if money>=500:
        ...
        if ct.fetchone():
            ...
            p=Pokemon(name=m[0],primaryType=m[1],secondaryType=m[2],level=random.randint(1,m[3]),hp=m[4],atk=m[5],defense=m[6],spatk=m[7],spdef=m[8],speed=m[9],moves=m[10], ability=m[11],sprite=m[12],gender=m[15],tera=tera,shiny=shiny,maxiv=maxiv,item=item)
            types=p.primaryType
            if p.secondaryType!="???":
