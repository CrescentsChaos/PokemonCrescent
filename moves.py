...
async def belch(ctx,x,y,tr1,em,field,turn):
    ...
    if "Berry[Used]" in x.item:
        ...
        em.add_field(name="No berry was used!",value="But it failed!")
#Poison Jab
...
async def quickattack(ctx,x,y,tr1,em,field,turn):
    ...
    y.hp-=await physical(x,x.level,x.atk,y.defense,base,a,b,c,r,al) 
#Accelerock
async def accelerock(ctx,x,y,tr1,em,field,turn):
    al=1
    r=await randroll()
    em.add_field(name=f"Move:",value=f"{x.name} used Accelerock!")    
    x.atktype="Rock"
    base=40
    if x.ability=="Technician":
        base*=1.5
